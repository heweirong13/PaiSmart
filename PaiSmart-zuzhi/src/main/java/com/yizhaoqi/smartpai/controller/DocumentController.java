...
public class DocumentController {
    ...
    public ResponseEntity<?> getAccessibleFiles(
        ...
        try {
            ...
            // 将FileUpload转换为包含tagName的DTO（与uploads接口格式保持一致）
            List<Map<String, Object>> fileData = files.stream().map(file -> {
                ...
                // 返回orgTag(tagId)供前端编辑使用
                dto.put("orgTag", file.getOrgTag());
                // 将orgTag从tagId转换为tagName用于列表显示
     ...
     *
     * @param fileMd5 文件MD5
     * @param body    请求体 { orgTag, isPublic }
     * @param role    用户角色（由过滤器注入）
     * @param userId  当前用户ID
     * @return 编辑结果
     */
    @PutMapping("/{fileMd5}")
    public ResponseEntity<?> updateDocument(
        ...
        LogUtils.PerformanceMonitor monitor = LogUtils.startPerformanceMonitor("UPDATE_DOCUMENT");
        try {
            // 仅管理员可编辑
            if (!"ADMIN".equals(role)) {
                monitor.end("编辑失败：权限不足");
                Map<String, Object> response = new HashMap<>();
                response.put("code", HttpStatus.FORBIDDEN.value());
                response.put("message", "仅管理员可编辑文档信息");
                return ResponseEntity.status(HttpStatus.FORBIDDEN).body(response);
            }

            Optional<FileUpload> fileOpt = fileUploadRepository.findByFileMd5(fileMd5);
            if (fileOpt.isEmpty()) {
                monitor.end("编辑失败：文档不存在");
                Map<String, Object> response = new HashMap<>();
                response.put("code", HttpStatus.NOT_FOUND.value());
                response.put("message", "文档不存在");
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(response);
            ...
            // 更新组织标签
            if (body.containsKey("orgTag")) {
                Object orgTagVal = body.get("orgTag");
                String newOrgTag = orgTagVal != null ? orgTagVal.toString() : null;
                file.setOrgTag(newOrgTag);

                // 同步更新 document_vectors 表中同一 fileMd5 的 org_tag
                documentVectorRepository.updateOrgTagByFileMd5(fileMd5, newOrgTag);
            }

            // 更新公开状态
            if (body.containsKey("isPublic")) {
                Object pubVal = body.get("isPublic");
                boolean newIsPublic = Boolean.TRUE.equals(pubVal);
                file.setPublic(newIsPublic);

                // 同步更新 document_vectors
                documentVectorRepository.updateIsPublicByFileMd5(fileMd5, newIsPublic);
            }

            fileUploadRepository.save(file);

            LogUtils.logFileOperation(userId, "UPDATE", file.getFileName(), fileMd5, "SUCCESS");
            monitor.end("文档编辑成功");

            Map<String, Object> response = new HashMap<>();
            response.put("code", 200);
            response.put("message", "文档信息更新成功");
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            LogUtils.logBusinessError("UPDATE_DOCUMENT", userId, "编辑文档失败: fileMd5=%s", e, fileMd5);
            monitor.end("编辑失败: " + e.getMessage());
            Map<String, Object> response = new HashMap<>();
            response.put("code", HttpStatus.INTERNAL_SERVER_ERROR.value());
            response.put("message", "编辑文档失败: " + e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
        }
    }
