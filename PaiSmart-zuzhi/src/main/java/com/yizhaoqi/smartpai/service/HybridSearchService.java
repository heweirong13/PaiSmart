        ...
        try {
            ...
            SearchResponse<EsDocument> response = esClient.search(s -> {
                        ...
                        s.knn(kn -> kn
                                ...
                                // 权限过滤放在 KNN 的 filter 中，确保只搜索有权限的文档
                                .filter(f -> f.bool(bf -> bf
                                        // 条件1: 用户可访问自己的文档
                                        .should(s1 -> s1.term(t -> t.field("userId").value(userDbId)))
                                        // 条件2: 公开文档
                                        .should(s2 -> s2.term(t -> t.field("isPublic").value(true)))
                                        // 条件3: 组织标签（排除公开文档和orgTag为null的文档）
                                        .should(s3 -> {
                                            if (userEffectiveTags.isEmpty()) {
                                                return s3.matchNone(mn -> mn);
                                            } else if (userEffectiveTags.size() == 1) {
                                                return s3.bool(inner -> inner
                                                    .must(m -> m.exists(e -> e.field("orgTag")))
                                                    .must(m -> m.term(t -> t.field("orgTag").value(userEffectiveTags.get(0))))
                                                );
                                            } else {
                                                return s3.bool(inner -> {
                                                    inner.must(m -> m.exists(e -> e.field("orgTag")));
                        ...
                        // 文本匹配 + 权限过滤（必须同时加权限过滤，因为 ES 中 knn 和 query 结果取并集）
                                ...
                                .filter(f -> f.bool(bf -> bf
                                        ...
                                        .should(s3 -> {
                                            if (userEffectiveTags.isEmpty()) {
                                                ...
                                                return s3.bool(inner -> inner
                                                    .must(im -> im.exists(e -> e.field("orgTag")))
                                                    .must(im -> im.term(t -> t.field("orgTag").value(userEffectiveTags.get(0))))
                                                );
                                            } else {
                                                return s3.bool(inner -> {
                                                    inner.must(im -> im.exists(e -> e.field("orgTag")));
                                                    inner.minimumShouldMatch("1");
                                                    userEffectiveTags.forEach(tag -> inner.should(sh2 -> sh2.term(t -> t.field("orgTag").value(tag))));
                                                    return inner;
                                                });
                            ...
                            .bool(b -> b
                                    ...
                                    .filter(f -> f
                                            .bool(bf -> bf
                                                    ...
                                                    // 条件3: 用户可以访问其所属组织的文档（包含层级关系，排除orgTag为null）
                                                    .should(s3 -> {
                                                        if (userEffectiveTags.isEmpty()) {
                                                            return s3.matchNone(mn -> mn);
                                                        } else if (userEffectiveTags.size() == 1) {
                                                            // 单个标签：必须存在orgTag且匹配
                                                            return s3.bool(innerBool -> innerBool
                                                                .must(m -> m.exists(e -> e.field("orgTag")))
                                                                .must(m -> m.term(t -> t
                                                                        .field("orgTag")
                                                            ...
                                                            // 多个标签：必须存在orgTag且匹配其中一个
                                                            return s3.bool(innerBool -> {
                                                                innerBool.must(m -> m.exists(e -> e.field("orgTag")));
